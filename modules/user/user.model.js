...
/* --------------------------- GET ------------------------- */
  ...
  adminLogin: async (con, email, password) => {
    var loginData = await con.query("SELECT u.u_id as \"userID\", u.u_name as name, u.u_lastname as \"lastName\", u.u_password as password, u.u_image as image, u.u_email as email, u.u_birthdate as birthdate, u.u_points as points, u.u_type as type, u.u_blocked as blocked, u.u_stripe_id as stripe_id, u.u_stripe_connect_id as stripe_connect_id, u_preferred_language as \"preferredLanguage\", u.fk_role_id as \"roleID\", u.fk_place_id as \"placeID\", p_name as place, u.fk_level_id as \"levelID\", r_name as \"roleName\", r_description as \"roleDescription\", l_name as \"levelName\", l_percentage as \"levelPercentage\", l_bonus as \"levelBonus\" FROM USER_F u, ROLE, LEVEL, PLACE WHERE u.u_email = '"+email+"' and u.u_password = '"+password+"' and u.fk_role_id = r_id and r_name = 'administrator' and u.fk_level_id = l_id and p_id = u.fk_place_id").catch((error) => {
      return new Error(error);
    });
    if(loginData.length === 0){
      return "Users doesn't exists.";
  ...
  createUser: async (con, address, user) => {
    ...
    if(userExists.length > 0){
      ...
      try {
        ...
        const user_id = await con.query('INSERT INTO USER_F(u_name, u_lastName, u_password, u_image, u_email, u_birthdate, u_points, u_type, u_blocked, fk_role_id, fk_place_id, fk_level_id, u_stripe_id, u_stripe_connect_id, u_preferred_language) VALUES($1, $2, $3, $4, $5, $6, 0, $7, false, 1, $8, 1, $9, $10, $11) RETURNING u_id',
